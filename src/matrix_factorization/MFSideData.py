...
def SGD(x):
    ...
    for v_ij in v_iter:
        ...
        print('Error for {}-th row and {}-th column is: {}'.format(i, j, error))
        # increment MSE
        mse += error**2
        # gradients with L2 loss
        w_i -= step_size.value*(-2*error*h_j + 2.0*reg.value*w_i)
        h_j -= step_size.value*(-2*error*w_i + 2.0*reg.value*h_j)
        print('W update is: {}; H update is: {}'.format(step_size.value*(-2*error*h_j + 2.0*reg.value*w_i), step_size.value*(-2*error*w_i + 2.0*reg.value*h_j)))
        # update entry
        w[i] = w_i
        h[j] = h_j
...
if __name__ == '__main__':
    ...
    w = v.map(lambda x: (x[0],1)).reduceByKey(lambda x,y: x+y).map(lambda x: (x[0], np.random.rand(k)/(k*5)))
    # initialize H
    ...
    h = v.map(lambda x: (x[1],1)).reduceByKey(lambda x,y: x+y).map(lambda x: (x[0], np.random.rand(k)/(k*5)))
